...
test("あなたに合う出会い方診断を最後まで進められる", async ({ page }) => {
  ...
  await expect(page.getByTestId("deai-fit-share-x-top")).toHaveAttribute("href", /share%3D20260627/);
...
test("result付きURLでは出会い方診断の個別結果カードを直接表示する", async ({ page }) => {
  ...
  await expect(page.locator("meta[property='og:image']")).toHaveAttribute("content", /\/og\/deai-fit\/fvsn\.png\?v=20260627/);
...
test("結果別URLではタイプ別OGPと診断導線を表示する", async ({ page }) => {
  ...
  await expect(page.locator("meta[property='og:image']")).toHaveAttribute("content", /\/og\/deai-fit\/ocqn\.png\?v=20260627/);
  await expect(page.locator("meta[name='twitter:image']")).toHaveAttribute("content", /\/og\/deai-fit\/ocqn\.png\?v=20260627/);
});
