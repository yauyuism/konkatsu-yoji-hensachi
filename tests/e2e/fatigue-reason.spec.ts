...
test("婚活疲れ・マチアプ疲れの理由診断を最後まで進められる", async ({ page }) => {
  ...
  await expect(page.getByTestId("fatigue-reason-card-map")).toContainText("詳しい婚活疲れマップ");
  await expect(page.getByTestId("fatigue-reason-share-card")).not.toContainText("ひとことで言うと");
  await expect(page.getByTestId("fatigue-reason-share-actions")).toContainText("この結果はXでシェアできます");
  await expect(page.getByTestId("fatigue-reason-share-actions")).toContainText("未対応の環境では、診断カードが表示される結果リンクを投稿します");
  await expect(page.getByTestId("fatigue-reason-share-actions")).toContainText("診断結果をXにシェア");
  await expect(page.getByTestId("fatigue-reason-share-actions")).toContainText("診断結果をもとに相談する");
...
test("結果別URLでは婚活疲れ診断の個別結果カードとOGPを直接表示する", async ({ page }) => {
  ...
  await expect(page.locator("meta[property='og:url']")).toHaveAttribute("content", /\/diagnoses\/konkatsu-fatigue\/result\/wrongPeople$/);
...
test("メイン共有ボタンは結果別URL付きのX投稿画面を開く", async ({ page }) => {
  ...
  expect(popupUrl).toContain("/diagnoses/konkatsu-fatigue/result/fastJudgment");
  await popup.close();
});
