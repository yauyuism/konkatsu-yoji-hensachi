...
test("婚活疲れ・マチアプ疲れの理由診断を最後まで進められる", async ({ page }) => {
  ...
  await expect(page.getByText("対応しているスマホでは、画像つきで共有できる画面が開きます。")).toBeVisible();
  await expect(page.getByText("診断カード画像をそのまま投稿に載せるための導線です。")).toBeVisible();
  await expect(page.getByTestId("fatigue-reason-save-card")).toContainText("画像を保存してシェア");
