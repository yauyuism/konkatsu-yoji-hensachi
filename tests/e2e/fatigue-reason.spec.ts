...
test("result付きURLでは婚活疲れ診断の個別結果カードを直接表示する", async ({ page }) => {
  ...
  await expect(page.getByText("この結果タイプの強さ")).toBeVisible();
  await expect(page.locator("meta[property='og:image']")).toHaveAttribute("content", /\/api\/og-fatigue-reason\?result=wrongPeople/);
