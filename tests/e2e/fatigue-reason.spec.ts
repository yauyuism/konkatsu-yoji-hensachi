...
test("婚活疲れ・マチアプ疲れの理由診断を最後まで進められる", async ({ page }) => {
  ...
  await expect(page.getByText("対応しているスマホでは、画像つきで共有できる画面が開きます。")).toHaveCount(0);
  await expect(page.getByText("診断カード画像をそのまま投稿に載せるための導線です。")).toHaveCount(0);
  await expect(page.getByTestId("fatigue-reason-save-card")).toContainText("診断結果をXにシェア");
